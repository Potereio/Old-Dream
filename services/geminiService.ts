...
function getFallbackPrompt(decade: string): string {
    return `Create a photograph of the person in this image as if they were living in the ${decade}. The photograph must capture the distinct fashion, hairstyles, and overall atmosphere of that time period. Place them in a NEW and UNIQUE background that is highly characteristic of the era. Ensure that besides the person, the main foreground elements (like objects, furniture, or props) are unique to this image and not repeated. The final image must be a clear photograph that looks authentic to the era.`;
}
