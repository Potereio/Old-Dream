...
export async function createAlbumPage(imageData: Record<string, string>): Promise<string> {
    ...
    ctx.font = `120px 'Zhi Mang Xing', cursive`;
    ctx.fillText('旧梦映像馆', canvasWidth / 2, 150);

    ctx.font = `50px 'Zhi Mang Xing', cursive`;
    ctx.fillStyle = '#555';
    ctx.fillText('Powered by Google', canvasWidth / 2, 240);
    ...
    reversedImages.forEach(({ decade, img }, reversedIndex) => {
        ...
        ctx.fillStyle = '#222';
        ctx.font = `80px 'Zhi Mang Xing', cursive`;
        ctx.textAlign = 'center';
